//! Embeds `resume-fingerprint.json` and `repos.json` into the WASM binary.
//!
//! The resume generator writes the real manifest to
//! `apps/frontend/generated/resume-fingerprint.json` and the `update-repos`
//! Trunk hook writes `apps/frontend/repos.json` before `trunk build` runs. When
//! either is absent (dev builds without resumes, a bare `cargo clippy`, or
//! `cargo test`), an empty default is substituted so the `include_str!`s in
//! `src/github.rs` always resolve.
