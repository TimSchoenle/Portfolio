...
license, toolchain and the docs/ index — come from
...
docker run --rm -p 8080:8080 \
  ...
  timschoenle/portfolio:v3.1.2
```

Run it from a checkout of this repository: the server refuses to start without an imprint and a
privacy notice, none is baked into the image, and `legal/` holds templates for both. Then open
<http://localhost:8080>. The runtime image is `FROM scratch`: one statically linked binary, the
client bundle beside it, and the configuration contract at `/config/contract.json`. It runs as
`1001:1001`, and nothing it needs has to be writable.
...
- Every route renders on the server and then hydrates. Per-route `<head>` metadata and JSON-LD ship
  ...
  serialized, so nothing arrives in the wrong language and gets swapped a moment later.
...
- **The legal documents are configuration, not code.** The imprint, the privacy notice and any
  ...
  both languages. `legal/` holds templates for both, with placeholders; the published texts are
...
- `/licenses` lists every crate the client and the server link, the license it ships under, and the
  verbatim text of every license file found. cargo-about produces it during the image build, and an
  unlisted license fails that build.
...
```bash
docker pull timschoenle/portfolio:v3.1.2
```
...
| `sentry.http_transactions` | `bool` | `PORTFOLIO_SENTRY__HTTP_TRANSACTIONS` | `true` | — | Record one Sentry transaction per request, named by the *matched route* rather than by the URI — so `/api/v1/legal/{slug}` does not become one transaction name per document. |
...
a pull request has to pass. Note what the license grants before you fork: reading the source and
